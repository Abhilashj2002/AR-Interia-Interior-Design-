...
let t = fs.readFileSync('main.ts', 'utf8').replace(/\r\n/g, '\n');
...
t = t.replace(oldShowroomEnd, newShowroomEnd);

// 8. Add CSS portfolio
if (!t.includes('@keyframes kenBurns') || true) {
  const oldPortEnd = `</div>
      </div>
    </div>
  </div>
  \${renderFooter()}
  ...
  t = t.replace(oldPortEnd, newPortEnd);
}
