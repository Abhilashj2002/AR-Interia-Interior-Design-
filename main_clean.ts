console.log("ANTIGRAVITY_MAIN_TS_LOADED");
import './index.css';
...
const getEnhancedChatbotResponse = (userMessage: string): { text: string; quickReplies?: string[]; bookingIntent: boolean; suggestionIntent: boolean } => {
  ...
  if (suggestionIntent) {
    ...
    const suggestionText = suggestions
      ? `${chatbotSettingsState.suggestionsIntro}\n\n${suggestions}\n\nTap **View Gallery** to explore more designs.`
...
let dashboardRenderRaf: number | null = null;
const renderStabilized = () => {
  if (state.activeTab !== 'dashboard') {
    render();
    return;
  }
  if (dashboardRenderRaf !== null) return;
  dashboardRenderRaf = window.requestAnimationFrame(() => {
...
const ADMIN_CHARTS_AUTO_REFRESH_MS = 5000;
...
const DEFAULT_PORTFOLIO_CONTENT: PortfolioContent = {
  founder: { name: 'Abhilash J', role: 'Founder & CEO', bio: '', photo: '' },
  coFounder: { name: 'Ramesh', role: 'Co-Founder', bio: '', photo: '' },
  designers: [],
  feedbackVideos: [],
  featuredProjects: [
    {
...
const getDefaultCalculatorImageLibrary = (): any[] => {
  ...
  return [...byCategory, ...byHomeRequirements].filter((item) => String(item.image || '').trim());
};
...
const getPackageDisplayImage = (pkg: any) => {
  ...
  const packageText = `${String(pkg?.name || '')} ${String(pkg?.subtitle || '')} ${String(pkg?.description || '')} ${String(pkg?.category || '')}`.toLowerCase();
  
  // If we have a primary image and it's not a known placeholder, ALWAYS use it
  // This ensures Apartment and Villa hero shots are shown on the home page as requested
  if (primary && !primary.includes('living1.jpg') && primary.length > 5) return primary;

  const prefersCategoryDrivenImage = /\b(villa|apartment|full\s*home|\d+\s*bhk)\b/.test(packageText);
  const isCategoryAsset = /\/category\//i.test(primary);
...
const navigateTo = (tab: TabKey, replace = false) => {
  ...
  // Ensure a category is selected when navigating to categories so the images view can render
  if (tab === 'categories' && (!state.customer.categories || state.customer.categories.length > 0) && !state.customer.activeCategory) {
    const firstCat = (state.customer.categories && state.customer.categories[0]);
...
const loadState = async () => {
  ...
  void Promise.allSettled([
    ...
    apiFetch('/api/packages', {}, 1800).then((response) => (response.ok ? response.json() : null))
  ...
  if (savedServices) {
    try {
      ...
      state.services = (Array.isArray(parsedServices) ? parsedServices : []).map((service: any, index: number) => ({
        ...
        video: String(service?.video || service?.image || '')
      }));
...
const normalizeFeedbackVideoEntries = (videos: any[]) => videos
  ...
  .filter((video: any) => {
    ...
    if (!url) return false;
    // Only keep explicit feedback uploads (admin upload endpoint stores under /uploads) or inline uploaded data URLs.
    if (/^data:video\//i.test(url)) return true;
    if (url.includes('/uploads/')) return true;
    // Prevent pulling generic library videos from /videos into feedback sections.
    return false;
  });
...
const refreshCustomerData = async (options: RefreshOptions = {}) => {
  ...
  try {
    ...
    if (bookingsData && Array.isArray(bookingsData.bookings)) {
      state.customer.bookings = bookingsData.bookings.map((booking: any) => normalizeBookingRecord(booking));
      ...
      .then((data) => {
        if (data && data.success && Array.isArray(data.customers)) {
          ...
          existingUsers.forEach((u: any) => {
            ...
            if (key) existingByKey.set(key, u);
          });
    ...
    apiFetch('/feedbacks', { headers: { ...getAuthHeaders() } })
      ...
      .then((data) => {
        if (data && data.success && Array.isArray(data.feedbacks)) {
          const mappedFeedbacks = data.feedbacks.map((fb: any) => ({
            id: fb.id,
            userId: fb.customerId,
  ...
  if (state.currentUser) {
    apiFetch(`/bookings?customerId=${encodeURIComponent(state.currentUser.id)}`, { headers: { ...getAuthHeaders() } })
      ...
      .then((data) => {
        if (data && data.success && Array.isArray(data.bookings)) {
          state.customer.bookings = data.bookings.map((b: any) => normalizeBookingRecord(b));
          if (shouldRenderRefreshResult(options)) renderStabilized();
        } else {
          state.customer.bookings = getBookings().map((booking: any) => normalizeBookingRecord(booking));
        }
      })
      .catch(() => {
        state.customer.bookings = getBookings().map((booking: any) => normalizeBookingRecord(booking));
      ...
      .then((data) => {
        if (data && data.success && Array.isArray(data.feedbacks)) {
          const mappedFeedbacks = data.feedbacks.map((fb: any) => ({
            ...
            userName: fb.userName || state.currentUser?.name || 'Customer',
            rating: fb.rating,
            comment: fb.comment,
            createdAt: fb.createdAt || new Date().toISOString()
          }));
...
const handlePaymentRedirect = async () => {
  ...
  try {
    ...
    try {
      ...
      updatePayment(paymentId, { status: statusResponse.status, metadata: { merchantTransactionId: transactionId } });
    }
    ...
    if (paymentStatus === 'paid') {
      setTimeout(() => {
        updateAdminCharts();
        console.log('✅ PhonePe payment verified and synced to admin dashboard');
      }, 100);
    }
  } catch (error) {
...
const ensureDislikeFeedbackSampleData = () => {
  ...
  const sampleUser = state.users.find((user) => user.role === 'customer') || state.currentUser;
...
const renderVideoEmbedOrTag = (url: string, classes: string, title = 'Video') => {
  const normalized = normalizeAssetUrl(String(url || '').trim());
  if (!normalized) return '';
...
const getEligibleBookingsForDiscount = () => {
  ...
  return (state.customer.bookings || [])
    ...
    .filter((booking: any) => normalizeBookingPaymentStatus(booking?.paymentStatus) !== 'paid')
      ...
      <div class="w-full max-w-6xl bg-white rounded-3xl overflow-hidden shadow-2xl relative my-8 max-h-[92vh]" data-action="ignore" data-modal-inner="true">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2">
          <div class="h-64 md:h-full overflow-hidden bg-slate-100 relative">
            <img src="${escapeHtml(getPackageDisplayImage(pkg))}" alt="${escapeHtml(pkg.name)}" class="w-full h-full object-cover motion-3d" data-motion3d="true" onerror="this.src='/category/Living room/living1.jpg'" />
          </div>
              ...
              ${pkg.rooms.map((room) => `
                <div class="group cursor-pointer" data-action="open-room-preview" data-room-id="${room.id}">
                  <div class="aspect-video rounded-2xl overflow-hidden bg-slate-200 relative mb-2 shadow-sm group-hover:shadow-md transition">
                    <img src="${escapeHtml(normalizeAssetUrl(room.image || ''))}" alt="${escapeHtml(room.title)}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500 motion-3d" data-motion3d="true" />
...
const calculatePrice = () => {
  ...
  const buildCuratedPackageDesigns = (pkg: any) => {
    ...
    [...featureDrivenDesigns].forEach((item: any) => {
  ...
  if (isFullHome) {
    ...
    const designMatches = allDesigns
      ...
      .slice(qualityOffset, qualityOffset + 5)
      .map((d, i) => {
...
const renderPriceCalculatorModal = () => {
  ...
  const displayConfigType = (Number(s.bhk) >= 4 || String(s.shape || '').toLowerCase() === 'custom') ? 'villa' : 'apartment';
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="space-y-8">
              ...
              <div>
                ...
                <div class="space-y-3">
                  ${['economy', 'premium', 'luxury'].map(q => `
                    <button data-action="calc-set-quality" data-value="${q}" class="w-full py-4 px-5 rounded-2xl border-2 text-left transition flex items-center justify-between group ${s.quality === q ? 'border-amber-400 bg-amber-50 shadow-sm' : 'border-slate-50 bg-slate-50/30 hover:border-slate-200'}">
                      <div>
                        <div class="font-black text-sm capitalize ${s.quality === q ? 'text-amber-700' : 'text-slate-700'}">${q}</div>
                        <div class="text-[10px] text-slate-400 uppercase font-bold tracking-tight">${q === 'economy' ? 'Standard Finishes' : q === 'premium' ? 'Superior Materials' : 'Ultra Premium Selection'}</div>
                      </div>
                      <div class="w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${s.quality === q ? 'border-amber-400 bg-amber-400 text-white scale-110' : 'border-slate-200 group-hover:border-slate-300'}">
                        ${s.quality === q ? '<i class="ri-check-line font-bold"></i>' : ''}
                      </div>
                    </button>
                  `).join('')}
                </div>
...
const renderInternal = () => {
  ...
  const content = renderLayout() + renderVideoModal() + renderPackageModal() + renderPriceCalculatorModal() + renderServiceDetailsModal() + renderServiceShowcaseDetailsModal();
  ...
  return `
    <div class="min-h-screen spidey-theme ${isHome ? 'bg-spidey' : 'bg-aurora'} text-slate-900">
      ...
      ${state.confirmMessage ? `
        ...
        <script>
          setTimeout(() => {
            const btn = document.querySelector('[data-action="close-confirm"]');
            if (btn) btn.click();
          }, 3000);
        </script>
      ` : ''}
...
const renderMain = () => {
  switch (state.activeTab) {
    ...
    case 'ai-studio':
      ...
      return renderAccessDenied('Sign in to access Design Studio.');
    case 'admin':
...
const renderHome = () => {
  ...
  const homepagePackages = allCustomerPackages
    ...
    .slice(0, 50);
  const packageCards = homepagePackages.length > 0 ? homepagePackages : allCustomerPackages.slice(0, 30);
    ...
    <section class="spidey-home">
        ...
        <div class="max-w-6xl mx-auto">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <button data-action="nav" data-tab="ai-studio" class="spidey-panel p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
              ...
              <h4 class="text-lg font-bold text-[color:var(--primary)] mb-2">3D Design Studio</h4>
              <p class="text-sm text-slate-500">Photorealistic 3D concepts, layouts, and walkthroughs to plan every detail of your dream space before execution.</p>
      ...
      <div class="mt-10 px-6">
        <div class="max-w-7xl mx-auto">
          ...
          <div class="flex flex-wrap justify-center gap-3 mb-8">
            <button class="px-4 py-2 rounded-full bg-[color:var(--primary)] text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => { const type=el.getAttribute('data-package-family'); const pkgtype=el.getAttribute('data-package-type'); el.style.display=(type!=='Villa' && pkgtype==='1BHK')?'':'none'; });">🏢 1BHK</button>
            <button class="px-4 py-2 rounded-full bg-[color:var(--primary)] text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => { const type=el.getAttribute('data-package-family'); const pkgtype=el.getAttribute('data-package-type'); el.style.display=(type!=='Villa' && pkgtype==='2BHK')?'':'none'; });">🏢 2BHK</button>
            <button class="px-4 py-2 rounded-full bg-[color:var(--primary)] text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => { const type=el.getAttribute('data-package-family'); const pkgtype=el.getAttribute('data-package-type'); el.style.display=(type!=='Villa' && pkgtype==='3BHK')?'':'none'; });">🏢 3BHK</button>
            <button class="px-4 py-2 rounded-full bg-[color:var(--primary)] text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => { const type=el.getAttribute('data-package-family'); const pkgtype=el.getAttribute('data-package-type'); el.style.display=(type!=='Villa' && pkgtype==='4BHK')?'':'none'; });">🏢 4BHK</button>
            <button class="px-4 py-2 rounded-full bg-gradient-to-r from-emerald-600 to-emerald-700 text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => el.style.display=el.getAttribute('data-package-family')==='Villa'?'':'none');">🏡 Villa</button>
            <button class="px-4 py-2 rounded-full bg-gradient-to-r from-blue-600 to-blue-700 text-white text-sm font-semibold" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => el.style.display=el.getAttribute('data-package-family')==='Apartment'?'':'none');">🏬 Apartment</button>
            <button class="px-4 py-2 rounded-full bg-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-300" onclick="Array.from(document.querySelectorAll('[data-package-family]')).forEach(el => el.style.display='');">📋 All</button>
          </div>
            ...
            ${packageCards.map((pkg: any) => {
              ...
              <div class="rounded-2xl border border-black/5 overflow-hidden bg-white hover:shadow-xl transition-all cursor-pointer has-3d-tilt" data-action="open-package-modal" data-package-id="${pkg.id}" data-package-type="${pkgFilterType}" data-package-family="${pkgType}" style="display: ">
                <div class="h-48 overflow-hidden bg-slate-100 relative">
                  <img src="${escapeHtml(getPackageDisplayImage(pkg))}" alt="${escapeHtml(pkg.name)}" class="w-full h-full object-cover" onerror="this.src='/category/Living room/living1.jpg'" />
      ...
      { userName: 'Sneha Reddy', rating: 5, comment: 'Luxury finish, practical planning, and strong attention to detail from start to finish.', createdAt: new Date().toISOString() }
              ...
              <div class="spidey-panel p-6 h-full flex flex-col">
                ...
                <div class="text-xs text-slate-400">${new Date(feedback.createdAt || Date.now()).toLocaleDateString()}</div>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      .map(
        (project) => `
            ...
            ? `<button type="button" data-action="open-service-showcase-details" data-showcase-id="${escapeHtml(String(project.id || ''))}" data-showcase-title="${escapeHtml(project.title || '')}" data-showcase-price="${escapeHtml(project.price || '')}" data-showcase-style="${escapeHtml(project.style || '')}" data-showcase-description="${escapeHtml(project.description || '')}" data-showcase-image="${escapeHtml(project.image || '')}" class="block w-full h-full text-left" title="View details">
                 <img src="${escapeHtml(project.image)}" alt="${escapeHtml(project.title)}" class="w-full h-full object-cover motion-3d cursor-pointer" data-motion3d="true" onerror="this.onerror=null;this.parentNode.innerHTML='<div class=\'w-full h-full bg-gradient-to-br from-slate-200 to-slate-400 flex items-center justify-center text-4xl\'>🏠</div>';" />
               </button>`
            ...
            ${getCustomerDisplayPackages().map((pkg: any) => `
              <div class="rounded-2xl border border-black/5 overflow-hidden bg-white hover:shadow-lg transition-shadow cursor-pointer has-3d-tilt" data-action="open-package-modal" data-package-id="${pkg.id}">
                <div class="h-48 overflow-hidden bg-slate-100">
                  <img src="${escapeHtml(getPackageDisplayImage(pkg))}" alt="${escapeHtml(pkg.name)}" class="w-full h-full object-cover" loading="lazy" onerror="this.src='/category/Living room/living1.jpg'" />
                </div>
                <div class="p-4 space-y-3">
                  <div class="text-xs uppercase tracking-[0.2em] text-slate-400">${escapeHtml(pkg.category || 'Package')}</div>
                  <h3 class="text-lg font-semibold text-[color:var(--primary)]">${escapeHtml(pkg.name)}</h3>
...
const renderPortfolio = () => {
  ...
  const normalizedCategories = categories.map((cat) => {
    ...
    return { id: categoryId, label, images, has3d, hasAr };
  });
                ...
                ${devs.map(d => `
                  <div class="bg-slate-800/60 backdrop-blur-md rounded-2xl border border-white/5 p-5 flex items-start gap-4 hover:bg-slate-800 transition-colors">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-slate-400 line-clamp-3">${escapeHtml(d.bio)}</p>
                    </div>
                  </div>
                `).join('')}
              </div>
      ...
      <div class="portfolio-showcase">
        <div class="portfolio-showcase-header">
          <div>
            ...
            <p class="text-slate-400 text-sm">Highlighted spaces with immersive 3D visualization</p>
          </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" >
  ${(() => {
    ...
    let featuredDesigns = getCustomerVisibleDesigns(allDesigns)
      ...
      // Add Sample Data for Villas and Apartments if viewing All
      if (portfolioCategoryId === 'all' && !portfolioSearchQuery) {
        ...
        // Cast to any to merge sample data which might miss some DesignModel props
      ...
      if (!featuredDesigns.length) {
        ...
        if (!fallbackImages.length) {
          return `<div class="col-span-full text-center py-16 text-slate-400 italic">No portfolio items match these filters.</div>`;
        }
          ...
          <div class="portfolio-card" style="animation-delay: ${(index % 10) * 0.1}s">
            <div class="portfolio-card-inner">
              ...
              <div class="p-6">
                ...
                <p class="text-slate-400 text-sm mb-4">Category image portfolio preview.</p>
                <div class="flex flex-wrap gap-2">
                  <span class="portfolio-tag">3D Design</span>
          ...
          <div class="portfolio-card-inner">
            <div class="portfolio-image-wrapper overflow-hidden relative group">
              ${design.previewImage ? `
                ...
                />
              ` : `
                <div class="w-full h-64 bg-slate-800 flex items-center justify-center text-slate-400">No image</div>
              `}
              <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center pointer-events-none group-hover:pointer-events-auto z-10 w-full h-full mt-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                <div class="flex gap-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 pointer-events-auto items-center mt-20">
                  <button 
                      ...
                      <!--Right side - Login Form-->
                        <div class="animate-fade-in-right" >
                          <div class="bg-white/98 backdrop-blur-2xl rounded-3xl shadow-2xl border border-amber-200/30 overflow-hidden" >
                            ${isSetupMode ? '<div class="bg-gradient-to-r from-amber-600 via-[color:var(--accent)] to-amber-700 text-white text-xs uppercase tracking-[0.3em] py-3 text-center font-bold">🔐 System Initialization</div>' : ''}
...
<div class="p-8 lg:p-10" >
  ...
  <p class="text-sm text-slate-600" >
    ${isSetupMode
      ? 'Set up your administrator account<br><span class="text-amber-600 font-semibold text-xs text-center block mt-1">Default: admin / admin123</span>'
      : state.loginForm.isSignup
...
<button 
                    type="submit"
class="w-full py-4 bg-gradient-to-r from-amber-600 via-[color:var(--accent)] to-amber-700 text-white font-bold rounded-xl shadow-lg hover:shadow-2xl transform hover:-translate-y-1 hover:scale-[1.02] transition-all relative overflow-hidden group"
  >
  <span class="relative z-10" > ${isSetupMode ? '🚀 Initialize System' : state.loginForm.isSignup ? '✨ Create Account' : '🔓 Sign In'} </span>
    <div class="absolute inset-0 bg-gradient-to-r from-amber-500 to-amber-800 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left" > </div>
      ...
      ? `<div class="mt-6 text-center">
                        ...
                        >
                          <span>${state.loginForm.isSignup ? '👈 Already have an account? Sign In' : '👉 New here? Create Account'}</span>
                        </button>
                      </div>`
      : ''
    }
...
const refreshAdminData = async (options: RefreshOptions = {}) => {
  ...
  try {
    if (!state.currentUser || state.currentUser.role !== 'admin') {
      console.warn('Not authorized to fetch admin data');
      return;
    }
    ...
    const [
      ...
      apiFetch('/ai/designs', { headers: authHeaders }, 6000),
      apiFetch('/customers', { headers: authHeaders }, 6000),
      apiFetch('/likes', { headers: authHeaders }, 6000),
      apiFetch('/feedbacks', { headers: authHeaders }, 6000),
      apiFetch('/enquiries', { headers: authHeaders }, 6000),
      apiFetch('/chatbot/history', { headers: authHeaders }, 6000)
    ]);
    ...
    if (bookingsResult.status === 'fulfilled' && bookingsResult.value.ok) {
      ...
      if (bookingsData && Array.isArray(bookingsData.bookings)) {
        state.admin.bookings = bookingsData.bookings.map((booking: any) => normalizeBookingRecord(booking));
...
const renderAdminDashboardSection = () => {
  ...
  const { categoryEarnings } = getAdminChartMetrics();
  const activeRevenueCategories = categoryEarnings.filter((item) => Number(item.earnings || 0) > 0).length;
  ...
  return `
    <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <div id="admin-revenue" class="spidey-panel p-6 flex items-center gap-4">
        ...
        <div>
          ...
          <div class="text-2xl font-bold text-[color:var(--primary)]">${formatCurrency(bookings.reduce((sum, b) => sum + getBookingAmount(b), 0))}</div>
        </div>
      </div>
    ...
    <section class="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-32">
      <div class="spidey-panel p-6">
    ...
    <section class="spidey-panel p-6 mt-32">
...
const renderAdminDesignEditor = () => {
  const isOpen = (state.admin as any).designEditOpen as boolean | undefined;
  if (!isOpen) return '';
  const previewImage = state.admin.designForm.images?.[0] || '';
  const currentModelUrl = String(state.admin.designForm.modelUrl || '').trim();
  return `
  <div class="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" data-action="close-design-editor">
    ...
    <div class="spidey-dialog-header">
        ...
        <form data-form="admin-design-edit" class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-600 mb-1">Upload 3D Model (Local)</label>
              <input name="modelFile" type="file" accept=".glb,.gltf,model/gltf-binary,model/gltf+json" class="w-full px-3 py-2 rounded-xl border border-black/10" />
...
const renderAdminBookingDetailsModal = () => {
  ...
  if (!booking) return '';
  const customer = state.users.find(u => u.id === booking.userId);
  const customerPhone = (customer as { phone?: string } | undefined)?.phone;
  const bookingContext = resolveBookingDesignContext(booking);
            ...
            <div class="space-y-4">
                ...
                <div class="border-t pt-4 space-y-3">
                    ...
                    <div>
                        ...
                        <div class="font-medium text-[color:var(--primary)]">${escapeHtml(customer?.name || 'Unknown')}</div>
                    </div>
                    <div>
                        <div class="text-sm text-slate-500">Email</div>
                        <div class="font-medium text-[color:var(--primary)]">${escapeHtml(customer?.email || 'N/A')}</div>
                    </div>
                    <div>
                        <div class="text-sm text-slate-500">Phone</div>
                        <div class="font-medium text-[color:var(--primary)]">${escapeHtml(customerPhone || 'N/A')}</div>
                    </div>
                </div>
...
const renderAdminBookingsSection = () => {
  ...
  const filteredBookings = bookings.filter((b) => {
    ...
    const bookingContext = resolveBookingDesignContext(b);
    const searchName = String(bookingContext.displayName || b.designName || '').toLowerCase();

    if (q && !searchName.includes(q)) return false;
    if (catId !== 'all' && bookingContext.categoryId !== catId) return false;
    if (pStatus !== 'all' && normalizeBookingPaymentStatus(b.paymentStatus) !== pStatus) return false;
    return true;
  });

  const stats = {
    total: bookings.length,
    paid: bookings.filter(b => normalizeBookingPaymentStatus(b.paymentStatus) === 'paid').length,
    pending: bookings.filter(b => normalizeBookingPaymentStatus(b.paymentStatus) === 'pending').length,
    approved: bookings.filter(b => isBookingApprovedForPayment(b)).length
  };

  return `
    <div class="spidey-panel p-6">
        <h2 class="text-xl font-display font-bold text-[color:var(--primary)] mb-6">📅 Bookings Manager</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
            <div class="relative">
              ...
              <input data-bind="admin.bookingFilter.query" value="${escapeHtml(state.admin.bookingFilter.query)}" placeholder="Search design..." class="w-full pl-10 pr-3 py-2.5 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
            </div>
            <select data-bind="admin.bookingFilter.categoryId" class="px-4 py-2.5 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                <option value="all">📂 All Categories</option>
                ${categories.map((cat) => `<option value="${cat.id}">${escapeHtml(cat.title)}</option>`).join('')}
            </select>
            <input data-bind="admin.bookingFilter.dateFrom" type="date" class="px-4 py-2.5 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
            <input data-bind="admin.bookingFilter.dateTo" type="date" class="px-4 py-2.5 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
            <select data-bind="admin.bookingFilter.paymentStatus" class="px-4 py-2.5 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                <option value="all">💰 All Payments</option>
                <option value="paid">✅ Paid</option>
                <option value="pending">⏳ Pending</option>
                <option value="failed">❌ Failed</option>
            </select>
            <button data-action="clear-booking-filters" class="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-600 font-semibold hover:bg-slate-200 transition">🔄 Clear Filters</button>
        </div>
      ...
      ? filteredBookings
        .map((booking) => {
          const bookingContext = resolveBookingDesignContext(booking);
          const bookingPaymentStatus = normalizeBookingPaymentStatus(booking.paymentStatus);
          const isPendingAction = isBookingActionPending(booking.id);
          const statusColors = {
                        ...
                        <div class="p-4 rounded-xl border border-black/10 bg-white hover:shadow-md transition">
                            <div class="flex items-start justify-between gap-3 mb-3">
                                <div class="flex items-start gap-3 min-w-0 flex-1">
                                  ...
                                  <div class="font-semibold text-[color:var(--primary)] text-sm">${escapeHtml(bookingContext.displayName)}</div>
                                    <div class="text-xs text-slate-400 mt-1">📅 ${new Date(booking.createdAt).toLocaleDateString()}</div>
                                    <div class="text-xs text-slate-500 mt-0.5">👤 ${escapeHtml(getBookingDisplayLabel(booking, bookingContext))}</div>
                                    </div>
                                </div>
...
const classifyRoomKindForDisplay = (titleLike: string, descriptionLike: string) => {
  const text = `${String(titleLike || '')} ${String(descriptionLike || '')}`.toLowerCase();
...
const sanitizePackageRoomsForDisplay = (rooms: PackageRoom[], packageId: string) => {
  ...
  return (Array.isArray(rooms) ? rooms : []).map((room: PackageRoom, index: number) => {
    const kind = classifyRoomKindForDisplay(room?.title || '', room?.description || '');
    ...
    if (hasRelevantImage) return room;
...
const getCustomerDisplayPackages = (): Package[] => {
  ...
  const buildGeneralLayout = (pkg: any) => {
    ...
    return [{ key: 'living', title: 'Living Area', aliases: ['Living Room'] }];
  };
  ...
  return (getPackagesSync() || []).map((rawPackage: any, packageIndex: number) => {
    ...
    const builtRooms: PackageRoom[] = (pkg.rooms && pkg.rooms.length > 0) 
      ? pkg.rooms 
      : targetLayout.map((item, roomIndex) => {
          const category = getCategoryByAliases(item.aliases);
          const roomImage = findImageForCategory(category, packageIndex + roomIndex + 1) || '/category/Living room/living1.jpg';
    ...
    // Get 20 related designs/images for this package
    const category = getCategoryByAliases(['Living Room', 'Kitchen', 'Bedroom']);
    const relatedDesigns = findRelatedDesignsForCategory(category, 20);
        ...
        <div class="mt-4 space-y-2 max-h-[400px] overflow-y-auto">
          ${filteredPackages.map(p => `
                <div class="p-3 rounded-xl border border-black/5 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      <img src="${escapeHtml(p.image || p.rooms?.[0]?.image || '/category/Living room/living1.jpg')}" class="w-10 h-10 rounded-lg object-cover" />
                        <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 items-center">
                      ...
                      <div class="flex items-center gap-2">
                        <div class="flex items-center gap-1">
                          <label class="text-[10px] font-bold text-slate-500">Upload:</label>
                          <input type="file" id="calc-img-upload-${index}" accept="image/*" class="text-xs file:mr-1 file:px-2 file:py-1 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-amber-50 file:text-amber-700 file:cursor-pointer" />
                        </div>
...
const renderAdminFeedbackSection = () => {
  ...
  return `
  <div class="spidey-panel p-6">
    <h2 class="text-xl font-display font-bold text-[color:var(--primary)] mb-4">⭐ Public Feedback</h2>
      ...
      ? uniqueFeedbacks
        .map((feedback) => `
                <div class="p-3 rounded-xl border border-black/5">
                  ...
                  <div class="text-[10px] text-slate-400 mt-2">${new Date(feedback.createdAt || Date.now()).toLocaleDateString()}</div>
                </div>
              `).join('')
...
const renderAdminPortfolioSection = () => {
  return `
    <section class="spidey-panel p-6 overflow-hidden">
        <h2 class="text-xl font-display font-bold text-[color:var(--primary)] mb-4">💼 Portfolio Content Manager</h2>
...
const renderAdminLuxuryEditorSection = () => {
  ...
  const allDesigns = state.customer.designs && state.customer.designs.length > 0 ? state.customer.designs : getDesigns();
    ...
    <section class="spidey-panel p-6">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    ${state.users.filter(u => u.role === 'customer').map(customer => {
    const customerLikes = state.customer.likes.filter(l => l.userId === customer.id && l.value === 'like').length;
    const customerFeedbacks = state.customer.feedbacks.filter(f => f.userId === customer.id).length;
    const customerBookings = allEffectiveBookings.filter((b: any) => String(b.userId || b.customerId || '') === String(customer.id)).length;
    return `
                        <tr class="border-b border-black/5 hover:bg-slate-50">
        ...
        <div class="mt-8">
            ...
            <div class="space-y-6">
                ${state.users.filter(u => u.role === 'customer').map(customer => {
    const customerLikes = state.customer.likes.filter(l => l.userId === customer.id && l.value === 'like');
    const customerFeedbacks = state.customer.feedbacks.filter(f => f.userId === customer.id);
    const likedDesignObjs = customerLikes.map(l => state.customer.designs.find(d => d.id === l.designId)).filter((d): d is DesignModel => !!d);
    return `
                    <div class="p-4 rounded-xl border border-black/5">
                        ...
                        <div>
                            ...
                            <div class="space-y-1">
                                ${customerFeedbacks.length ? customerFeedbacks.map(f => `<div class="text-xs p-2 rounded-lg bg-slate-50 text-slate-600">${'★'.repeat(f.rating)} - ${escapeHtml(f.comment)}</div>`).join('') : '<span class="text-xs text-slate-400">No feedback yet</span>'}
                            </div>
                        </div>
                    </div>
                    `;
...
const renderCustomerDashboard = () => {
  ...
  const filteredPackages = packageTypeFilter === 'All'
    ? dashboardPackages
    : dashboardPackages.filter((p: Package) => String(p.category || '') === packageTypeFilter);
  ...
  return `
    <div class="spidey-page">
      ...
      <section class="spidey-grid lg:grid-cols-[1fr_2fr] gap-8">
        <div class="space-y-6">
          <div class="spidey-panel p-6">
        ...
        <div class="space-y-6">
          <div class="spidey-panel p-6">
            <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
              ...
              <span class="text-xs text-slate-400">${filteredPackages.length} packages</span>
            </div>
            <!-- Type filter chips -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              ${filteredPackages.length > 0 ? filteredPackages.map((pkg: Package) => {
                ...
                <div class="rounded-2xl border border-black/10 bg-white overflow-hidden flex flex-col group hover:shadow-md transition has-3d-tilt">
                  <div class="relative h-40 bg-slate-100 overflow-hidden">
                    <img src="${escapeHtml(getPackageDisplayImage(pkg))}" alt="${escapeHtml(pkg.name)}" class="motion-3d w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" data-motion3d="true" onerror="this.src='/category/Living room/living1.jpg'" />
          ...
          <div class="spidey-panel p-6">
            <h3 class="text-lg font-bold text-[color:var(--primary)] mb-4">Recent Bookings</h3>
            <div class="space-y-4">
              ${bookings.length > 0 ? bookings.map((booking) => {
      const bookingContext = resolveBookingDesignContext(booking);
                  const bookingDisplayLabel = getBookingDisplayLabel(booking, bookingContext);
      const bookingPaymentStatus = normalizeBookingPaymentStatus(booking.paymentStatus);
      const paymentBadgeClass = bookingPaymentStatus === 'paid'
        ? 'bg-emerald-100 text-emerald-700'
                ...
                <div class="p-4 rounded-xl border border-black/5 bg-white shadow-sm">
                  <div class="flex justify-between items-start mb-3">
                    <div class="flex items-start gap-3 min-w-0">
                      <img src="${escapeHtml(bookingContext.imageUrl)}" alt="${escapeHtml(bookingContext.displayName)}" class="w-14 h-14 rounded-xl object-cover border border-black/10" />
                      <div class="min-w-0">
                      <div class="font-bold text-[color:var(--primary)] truncate">${escapeHtml(bookingContext.displayName)}</div>
                      <div class="text-xs text-slate-500">${escapeHtml(bookingDisplayLabel)} • ${new Date(booking.createdAt).toLocaleDateString()}</div>
                      </div>
                    </div>
                    <div class="text-right">
                      <div class="font-bold text-[color:var(--accent)]">${formatCurrency(getBookingAmount(booking))}</div>
                      <div class="text-xs font-semibold px-2 py-1 rounded-full inline-block mt-1 ${booking.status === 'confirmed' || booking.status === 'fulfilled' ? 'bg-green-100 text-green-700' :
          booking.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
        }">${escapeHtml(booking.status)}</div>
                      <div class="text-xs font-semibold px-2 py-1 rounded-full inline-block mt-1 ${paymentBadgeClass}">${bookingPaymentStatus === 'paid' ? '✓ Paid' : bookingPaymentStatus === 'failed' ? '✗ Failed' : '⏳ Pending'}</div>
                    </div>
                  </div>
                  <div class="flex gap-2 mt-4 pt-3 border-t border-slate-100">
                    ...
                    ` : ''}
                    ${bookingPaymentStatus !== 'paid' && !isBookingApprovedForPayment(booking) ? `
                      <div class="flex-1 py-2 rounded-lg bg-amber-50 text-amber-700 text-xs font-semibold text-center border border-amber-200">Waiting for admin approval</div>
                    ` : ''}
              ...
              `;
    }).join('') : '<div class="text-center py-10 text-slate-400 italic">No bookings found. Explore our gallery to book a design!</div>'}
            </div>
          </div>
        </div>
      </section>
      ...
      return mine.slice(0, 8).map((item: any) => `
                  <div class="p-3 rounded-xl border border-black/5 bg-slate-50">
                    <div class="flex items-center justify-between mb-1">
                      ...
                      <div class="text-[10px] text-slate-400">${item.createdAt ? new Date(item.createdAt).toLocaleDateString() : ''}</div>
                    </div>
      ...
      <div class="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-fade-in-up" data-action="ignore">
        ...
        <form data-form="catalog-upload" class="p-6 space-y-5">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
            ...
            <div class="relative group cursor-pointer">
              <input type="file" name="glb" accept=".glb,.gltf" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" data-file-input="glb" />
              <div class="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center group-hover:border-[color:var(--primary)] transition bg-slate-50">
                <div class="text-2xl mb-2">🧊</div>
                <div class="text-xs font-bold text-slate-600" data-file-name="glb">Upload 3D Model</div>
                <div class="text-[10px] text-slate-400 mt-1">GLB, GLTF</div>
              </div>
            </div>
          </div>
...
const renderAdmin = () => {
  ...
  return normalizeTemplateMarkup(`
    <div class="spidey-page">
      ...
      <div class="admin-dashboard-metrics mt-32">
        ${renderAdminDashboardSection()}
      </div>

      <section class="admin-masonry-layout mt-32">
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminBookingsSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminInquiriesSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminCategoriesSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminDesignManagerSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminCatalogSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminPackageManagerSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminCalculatorSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminFeedbackSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminDislikeFeedbackSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminChatbotManagerSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminAnnouncementsSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminThemeSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminDesignStudioSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminServicesSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminPortfolioSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half">${renderAdminLuxuryEditorSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half" id="admin-showrooms">${renderAdminShowroomsSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--half" id="admin-showcases">${renderAdminShowcaseSection()}</div>
        <div class="admin-masonry-item admin-masonry-item--full admin-masonry-item--customers">${renderAdminCustomersSection()}</div>
      </section>
...
const getAdminChartMetrics = () => {
  ...
  const resolveCategoryId = (booking: any) => {
    ...
    const candidateValues = [bookingCategoryRaw, designCategoryRaw].filter(Boolean);
  ...
  bookings.forEach((booking: any) => {
    ...
    const label = created.toLocaleString('en-US', { month: 'short', year: 'numeric' });
...
const updateAdminCharts = () => {
  ...
  if (charts.category) {
    const hasRevenueData = categoryEarnings.some((c) => Number(c.earnings || 0) > 0);
    const labels = hasRevenueData ? categoryEarnings.map((c) => c.categoryTitle) : ['No Revenue Data'];
    const data = hasRevenueData ? categoryEarnings.map((c) => c.earnings) : [1];
    const colors = hasRevenueData ? getCategoryChartColors(labels.length) : ['#cbd5e1'];
...
const handleAdminChartsVisibilityChange = () => {
  ...
  if (!document.hidden) {
    void refreshAdminData({ silent: true }).finally(() => updateAdminCharts());
  }
};
...
const initAdminCharts = async () => {
  try {
    ...
    if (categoryCanvas) {
      ...
      const labels = hasRevenueData ? categoryEarnings.map((c) => c.categoryTitle) : ['No Revenue Data'];
      const data = hasRevenueData ? categoryEarnings.map((c) => c.earnings) : [1];
      const colors = hasRevenueData ? getCategoryChartColors(labels.length) : [theme.emptyCategory];
...
const handleLogin = async (formData: FormData) => {
  ...
  try {
    ...
    try {
      if (!loginResponse.ok) {
        try {
          ...
          throw new Error(`Server returned ${loginResponse.status}: ${loginResponse.statusText} `);
        }
      ...
      if (localMatch) {
        state.currentUser = { id: localMatch.id, name: localMatch.name, email: localMatch.email, role: localMatch.role || 'customer', password } as User;
        syncCustomerPackagePreferences(state.currentUser.id);
        persistState();
        refreshCustomerData();
    ...
    if (!loginData.success) {
      // Backend login failed — try local users fallback
      const localMatch = state.users.find((u) => (u.email === emailOrUsername || u.name === emailOrUsername || u.id === emailOrUsername || (u as any).username === emailOrUsername) && (u as any).password === password);
      if (localMatch) {
        state.currentUser = { id: localMatch.id, name: localMatch.name, email: localMatch.email, role: localMatch.role || 'customer', password } as User;
        syncCustomerPackagePreferences(state.currentUser.id);
    ...
    if (loginData.customer.role === 'admin') {
      ...
      if (state.currentUser) {
        const existing = state.users.find((u) => u.id === state.currentUser?.id);
        if (!existing) {
          state.users.push(state.currentUser);
        }
      }
...
const handlePublicFeedbackSubmit = (formData: FormData) => {
  const name = String(formData.get('name') || '').trim();
...
const handleBookDesignOnly = async (designId: string) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      setTimeout(() => {
        updateAdminCharts();
        console.log('✅ Book-only flow synced to admin dashboard');
      }, 100);
    } else {
    ...
    const rzpOptions = {
      ...
      handler: async (response: any) => {
        ...
        if (verifyData.success) {
          ...
          setTimeout(() => {
            updateAdminCharts();
            console.log('✅ Razorpay payment synced to admin dashboard');
          }, 100);
        } else {
...
const submitCardPaymentWithProvider = async (cardNumber: string, cvv: string, name: string) => {
  ...
  try {
    ...
    if (fp.isServer) {
      try {
        await apiFetch('/payments/fake/complete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
          body: JSON.stringify({
            bookingId,
    ...
    // Always update local store so UI reflects payment immediately
    updatePayment(paymentId, { status: 'success', amount, metadata });
    updateBooking(bookingId, { status: 'confirmed', paymentStatus: 'paid', price: amount });
    ...
    setTimeout(() => {
      updateAdminCharts();
      console.log('✅ Simulated card payment marked paid and synced to dashboards');
    }, 100);
...
const updateDesignForm = (designId: string) => {
  ...
  if (!design) return;
  const formImages = (design.images && design.images.length > 0)
    ? design.images
...
const openAdminDesignEditor = (designId: string | null, modelId: string | null) => {
  ...
  if (byDesign) {
    ...
    updateDesignForm(byDesign.id);
    return;
  }

  const model = modelId ? state.catalog.find((m) => m.id === modelId) : null;
  if (!model) return;
  const allCategories = [...getCategories(), ...(state.customer.categories || [])];
  const matchedCategory = allCategories.find((cat) => {
    const catLabel = (cat.title || cat.name || '').toLowerCase();
    return catLabel && catLabel === String(model.category || '').toLowerCase();
  });
  const resolvedCategoryId = model.categoryId || matchedCategory?.id || allCategories[0]?.id || '';

  (state.admin as any).designEditOpen = true;
  const formImages = model.previewImage ? [model.previewImage] : [];
  state.admin.designForm = {
    id: model.id,
    title: model.title,
    description: model.description || '',
    categoryId: resolvedCategoryId,
    price: getDesignAmount(model as any),
    cost: model.cost || 0,
...
root.addEventListener('click', async (event) => {
  ...
  switch (action) {
    ...
    case 'play-video': {
      ...
      if (videoUrl) {
        state.currentVideos = [{ url: videoUrl, title: videoTitle }];
        state.currentVideoUrl = toYouTubeEmbedUrl(videoUrl, true);
    ...
    case 'ai-generate': {
      ...
      (async () => {
        try {
          ...
          const categoryName = roomToCategoryMap[state.smartStudio.room] || 'Living room';
    ...
    case 'ai-save': {
      ...
      (async () => {
        try {
          ...
          if (!resp.ok) throw new Error('Failed to save designs');
    ...
    case 'admin-add-service': {
      ...
      const video = normalizeAssetUrl(String(formData.get('video') || '').trim());
      ...
      if (imageFile) {
        ...
        (async () => {
          try {
            ...
            await createShowcase(imageUrl);
            state.serviceShowcases = getServiceShowcases().map((showcase: any, index: number) => normalizeShowcaseRecord(showcase, index));
            state.admin.showcaseForm = {};
    ...
    case 'admin-update-showcase': {
      ...
      updateServiceShowcase(id, { title, style, duration, price, description, image: imageUrl, serviceId, relatedImages });
      state.serviceShowcases = getServiceShowcases().map((showcase: any, index: number) => normalizeShowcaseRecord(showcase, index));
      state.admin.showcaseForm = {};
    ...
    case 'book-ai-design': {
      ...
      (async () => {
        try {
          ...
          // Refresh customer data to show the new booking
          await refreshCustomerData();
          await refreshAdminData();
          setTimeout(() => {
            updateAdminCharts();
            console.log('✅ AI design booking synced to admin dashboard');
          }, 100);
...
const fetchPackagesFromServer = async () => {
  try {
    ...
    if (data.success && Array.isArray(data.packages)) {
      // For each package, fetch its individual designs (rooms)
      const fullPackages = await Promise.all(
        data.packages.map(async (pkg: any) => {
          try {
            ...
            return {
              ...pkg,
              rooms: dData.success && Array.isArray(dData.designs) ? dData.designs : []
            };
          } catch (e) {
            console.warn(`Failed to fetch designs for package ${pkg.id}`, e);
            return pkg;
          }
        })
      );
      setPackages(fullPackages);
      console.log('✅ Synchronized packages from server');
    }
  } catch (error) {
...
const init = async () => {
  ...
  try {
    ...
    // Synchronize data from server if possible
    await Promise.all([
